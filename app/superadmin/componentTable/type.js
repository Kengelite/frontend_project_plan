...
export default function DatatableType({ onEdit }) {
  ...
  const handleDelete = async (row) => {
    ...
    const result = await Swal.fire({
      ...
      text: `คุณต้องการคุณต้องการลบ "${row.style_name}" หรือไม่
        `,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await DeleteStyle(token, row.style_id);
        // if(response)
        console.log(response);
        if (response) {
          // setData((prevData) =>
          //   prevData.filter((item) => item.style_id !== row.style_id)
          // );
        ...
        <>
          <div style={{ padding: "5px" }}>
            <button
              ...
              onClick={() => {
                onEdit(row.style_name, row.style_id);
              }}
            >
  ...
  useEffect(() => {
    const filtered = data.filter((data) => {
      return `${data.style_name} ${data.project_number}`
        .toLowerCase()
  ...
  const handlechageStatus = async (row) => {
    ...
    if (result.isConfirmed) {
      try {
        const token = Cookies.get("token");
        const response = await UpdatestatusStyle(token, row.style_id);
        // if(response)
        console.log(response);
        if (response) {
          ...
          Swal.fire({
            ...
            // text: ` ${newStatus === 1 ? "เปิดการใช้งาน" : "ปิดการใช้งาน"} ${row.style_name}`,
  ...
  const fetchData = useCallback(async (page = 1, perPage = 10) => {
    try {
      setLoading(true);
      const token = Cookies.get("token");
      const res = await GetDatastyleall(token, page, perPage);
      setData(res.data);
