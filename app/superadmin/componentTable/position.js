...
export default function DatatablePosition({ onEdit }) {
  ...
  const handleDelete = async (row) => {
    ...
    const result = await Swal.fire({
      ...
      text: `คุณต้องการคุณต้องการลบ "${row.position_name}" หรือไม่
        `,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await DeletePosition(token, row.position_id);
        // if(response)
        console.log(response);
        if (response) {
          // setData((prevData) =>
          //   prevData.filter((item) => item.position_id !== row.position_id)
          // );
        ...
        <>
          <div style={{ padding: "5px" }}>
            <button
              ...
              onClick={() => {
                onEdit(row.position_name, row.position_id);
              }}
            >
  ...
  useEffect(() => {
    const filtered = data.filter((data) => {
      return `${data.position_name} ${data.project_number}`
        .toLowerCase()
  ...
  const handlechageStatus = async (row) => {
    ...
    const result = await Swal.fire({
      ...
      text: `คุณต้องการ  ${newStatus === 1 ? "เปิดการใช้งาน" : "ปิดการใช้งาน"}
        สำหรับ  "${row.position_name}" หรือไม่
        `,
      icon: "warning",
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      try {
        const token = Cookies.get("token");
        const response = await UpdatestatusPosition(token, row.position_id);
        // if(response)
        console.log(response);
        if (response) {
          ...
          setData((prevData) =>
            prevData.map((item) =>
              item.position_id === row.position_id
          ...
          Swal.fire({
            ...
            // text: ` ${newStatus === 1 ? "เปิดการใช้งาน" : "ปิดการใช้งาน"} ${row.position_name}`,
  ...
  const fetchData = useCallback(async (page = 1, perPage = 10) => {
    try {
      setLoading(true);
      const token = Cookies.get("token");
      const res = await GetDatapositionall(token, page, perPage);
      setData(res.data);
